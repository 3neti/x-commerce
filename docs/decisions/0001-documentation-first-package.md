...
x-commerce is intended to become the commercial architecture and reusable package for packaging, pricing, licensing, metering, and sustaining outcomes delivered through the x-change ecosystem.
...
x-commerce will begin as a reusable Laravel package with documentation-first scope.
...
Production commerce abstractions are deferred.

Commercial extraction from x-change is deferred.

Future extraction requires characterization, public API review, compatibility constraints, tests, ownership-boundary documentation, and a separate decision record.
...
The initial PHP implementation is intentionally minimal.

This package will not yet contain pricing engines, catalogs, carts, checkout, orders, invoices, licensing enforcement, transaction billing, bank integrations, NetBank integrations, or x-change integrations.
